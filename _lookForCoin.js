...
async function lookForCoinFunc() {
	
	var d = new Date();
	console.log('look for coin Time Start:'+d);
...
if (!firebase.apps.length) {
	...
	var snapSimBusy = (snapshot_W.val() && snapshot_W.val().usdt_simulationBusy) || [];
...
console.log('-      -      -      -      -      -      -')
console.log('LookForCoin: listOfArguments LENGTH:'+listOfArguments.length);
console.log('-      -      -      -      -      -      -')
...
const asyncOperation = index => {
  ...
  return new Promise((resolve, reject) => {
	  ...
	  ////-------
	  //console.log('lookForCoin: index[0] sent:'+index[0]);
	  ...
	  const controller = new AbortController();
		...
		if ( index[2]=='depth' ){
			...
			//console.log(' j_prom j_prom j_prom:');
				//console.log(j_prom);
...
console.log('lookForCoin: About to run promises...');
...
for (s=0; s<klines30mArr.length; s++) {
	 ...
	 //if ((klines30mArr[s][0].price>0.0000011) && (klines30mArr[s][0].vol>0) && (klines30mArr[s][0].vol_BTC>300) && (klines30mArr[s][0].chart =='30m') ) {
		...
		/*  klines30mArr[s][0].lP_ = lP_;
		 ...
		 //var lP_Opposite = parseFloat(orderBook_D.bids[1][0]);
		// console.log('lP_: :'+lP_)
						...
						//-----------------------------------------------------------------------------------		
								...
								//.....................................
									...
									//	&& ((((candlesForCoin_[u][4]/ma20TestA)-1)*100)<5)
										...
										//console.log('Entered Long Position at '+moment(candlesForCoin_[u][6]).format('YYYY-MM-DD HH:mm:ss') );
									}
									
									var ma20TestA_LowBar = ma20TestA*(0.98);
									if (  ((candlesForCoin_[u][4]<ma20TestA_LowBar) && (holdingCoin==1))   ) { //|| ((holdingCoin==1) && (u==candlesForCoin_.length-1) )
										...
										//console.log('LEAVING Position at '+perRet+'% at '+moment(candlesForCoin_[u][6]).format('YYYY-MM-DD HH:mm:ss') );
...
//console.log('filteredList BELOW...');
//console.log(filteredList);
...
for (var r=0; r<filteredList.length; r++){
	...
	console.log('...')
						...
						//filteredList.sort(orderByProperty('calc_inverseRSI', 'calc_macd'));
						...
						console.log('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX')
							console.log('LOOK FOR COIN:: FILTERED LIST  ReverseData FIRST: AFTER: '+ReverseData.length);
							
						
						/* if (ReverseData.length>=1) {
									var fs_p = require('fs').promises;
									try {
										await fs_p.writeFile(sourceData, JSON.stringify(ReverseData));
...
var queryBook = [];
	...
	console.log('LONG ReverseData below:');
...
for (let i = 0; i < queryBook.length; i++) {
  if (queryBook[i][0]=='ADD_COIN'){
	  console.log('ReverseData.length: '+ReverseData.length);
	  if (ReverseData.length>=1) {
			var count_B = 0;
			var selectedCoin ='';
			...
			while((count_B<ReverseData.length) && (selectedCoin=='') ){
				...
				//console.log(ReverseData[count_B].symbol+' CHART:'+ReverseData[count_B].chart+' RSI::'+goodRSI+' PPO::'+goodPPO+' Type:'+short_long+' oneUnitTest::'+oneUnitTest+' PD:'+(((ReverseData[count_B].lP_B/ReverseData[count_B].price)-1)*100)+' goodPriceRange:'+goodPriceRange);
				...
				//console.log('L: '+ReverseData[count_B].symbol+' RSI:'+ReverseData[count_B].rsi+' conditionsMet:'+conditionsMet+'  ---  A:'+((parseFloat(ReverseData[count_B].roc20Arr[0])>parseFloat(ReverseData[count_B].roc20Arr[1])) &&  (parseFloat(ReverseData[count_B].roc50Arr[0])>parseFloat(ReverseData[count_B].roc50Arr[1]))   &&  (parseFloat(ReverseData[count_B].roc100Arr[0])>parseFloat(ReverseData[count_B].roc100Arr[1])) &&  (parseFloat(ReverseData[count_B].roc200Arr[0])>parseFloat(ReverseData[count_B].roc200Arr[1]))) +' B:'+  (!((parseFloat(ReverseData[count_B].ma20)<parseFloat(ReverseData[count_B].ma50)) && (parseFloat(ReverseData[count_B].ma50)<parseFloat(ReverseData[count_B].ma100))))  + ' C:'+  ((((parseFloat(ReverseData[count_B].closePrice)/parseFloat(ReverseData[count_B].lowPrice))-1)*100) < 1 )+ ' D:'+ (((((parseFloat(ReverseData[count_B].closePrice)/parseFloat(ReverseData[count_B].lowPriceEight))-1)*100) >= -1 ) && ((((parseFloat(ReverseData[count_B].closePrice)/parseFloat(ReverseData[count_B].lowPriceFour))-1)*100) >= -0.5 ))    )
				console.log('L: '+ReverseData[count_B].symbol+' RSI:'+ReverseData[count_B].rsi+' conditionsMet:'+conditionsMet+' --- A:'+((parseFloat(ReverseData[count_B].roc20Arr[0])>parseFloat(ReverseData[count_B].roc20Arr[1])) &&  (parseFloat(ReverseData[count_B].roc50Arr[0])>parseFloat(ReverseData[count_B].roc50Arr[1]))   &&  (parseFloat(ReverseData[count_B].roc100Arr[0])>parseFloat(ReverseData[count_B].roc100Arr[1])) &&  (parseFloat(ReverseData[count_B].roc200Arr[0])>parseFloat(ReverseData[count_B].roc200Arr[1])))+' & B:'+(!( (parseFloat(ReverseData[count_B].ma20)<parseFloat(ReverseData[count_B].ma50)) && (parseFloat(ReverseData[count_B].ma50)<parseFloat(ReverseData[count_B].ma100)) ))+' & C:'+((((parseFloat(ReverseData[count_B].closePrice)/parseFloat(ReverseData[count_B].lowPrice))-1)*100) < 1 )+' & D:'+(((((parseFloat(ReverseData[count_B].closePrice)/parseFloat(ReverseData[count_B].closePriceEight))-1)*100) >= -1 ) && ((((parseFloat(ReverseData[count_B].closePrice)/parseFloat(ReverseData[count_B].closePriceFour))-1)*100) >= -0.5 ))+' & E:'+((parseFloat(ReverseData[count_B].ppo[0])>parseFloat(ReverseData[count_B].ppo[1])) && (parseFloat(ReverseData[count_B].ppo[1])>parseFloat(ReverseData[count_B].ppo[2])))+' & F:'+((((parseFloat(ReverseData[count_B].prevOpenPrice)/parseFloat(ReverseData[count_B].prevClosePrice))-1)*100)<2) )
				
				//console.log(ReverseData[count_B].symbol+' '+(((parseFloat(ReverseData[count_B].closePrice)/parseFloat(ReverseData[count_B].lowPriceEight))-1)*100) +' & '+(((parseFloat(ReverseData[count_B].closePrice)/parseFloat(ReverseData[count_B].lowPriceFour))-1)*100))
				//&& (oneUnitTest<3) OLD
				var closePriceMore = parseFloat(ReverseData[count_B].closePrice)*1.01;
				if ( ( (closePriceMore>1.02) || (closePriceMore<0.98) ) &&  (ReverseData[count_B].ma200 != 0)  && (baseMatch=='USDT') && (baseMatch==queryBook[i][3]) && (conditionsMet==1) &&  (ReverseData[count_B].symbol!= 'SUSDUSDT') &&  (ReverseData[count_B].symbol!= 'ORNUSDT')  &&  (ReverseData[count_B].symbol!= 'DREPUSDT')   &&  (ReverseData[count_B].symbol!= 'SUSHIUSDT') && (ReverseData[count_B].symbol.includes("UPUSDT") == false ) && (ReverseData[count_B].symbol.includes("DOWN") == false )   ) { 
					...
					console.log('COUNT___>'+count_B+' & SYMBOL: +'+ReverseData[count_B].symbol+' baseMatch:'+baseMatch);
					...
					var ws_json_B = queryBook[i][4]; // _B
					...
					if ((addCoinWs==1) && (short_long=='long') ){
						...
						if (snapActSim==0){
							...
							for (a=0; a<ws_ws_json.length; a++){
								if ( ReverseData[count_B].symbol == ws_ws_json[a].symB ){
									addThistoAcc=0;
									console.log('ACC UPDATE::: '+ReverseData[count_B].symbol+' Matched so not adding it again..');
									selectedCoin ='';
								}
							}
								...
								if ((canShort!=1) && (l_flipA==-1) ){
									addThistoAcc=0;
								}
							
							if ((addThistoAcc==1) && (ws_ws_json.length<2)) {
								selectedCoin =ReverseData[count_B].symbol;
								
								if (l_flipA==1){
									ws_json_B.push({'symB':ReverseData[count_B].symbol,  'monitorButtonText':'Stop Monitoring', 'initialPrice':closePriceMore, 'discountPercent':discountPercent.toString(), 'baseBalance':queryBook[i][2], 'date':now.format('YYYY-MM-DD HH:mm:ss'), 'chart':ReverseData[count_B].chart, 'chart_c':ReverseData[count_B].chart_c,   'model':ReverseData[count_B].model,  'type':'LONG',  'closePrice':ReverseData[count_B].closePrice, 'lowPrice':ReverseData[count_B].lowPrice, 'highPrice':ReverseData[count_B].highPrice,   'rsi':ReverseData[count_B].rsi, 'flipped':'no'   });
									firebase.database().ref('/Ins/'+queryBook[i][1]+'_B').set({
										ws_key_B: JSON.parse( JSON.stringify(ws_json_B) ),
									})
								}
								else if (l_flipA==-1){
									ws_json_B.push({'symB':ReverseData[count_B].symbol,  'monitorButtonText':'Stop Monitoring', 'initialPrice':closePriceMore, 'discountPercent':discountPercent.toString(), 'baseBalance':queryBook[i][2], 'date':now.format('YYYY-MM-DD HH:mm:ss'), 'chart':ReverseData[count_B].chart, 'chart_c':ReverseData[count_B].chart_c,   'model':ReverseData[count_B].model,  'type':'LONG',  'closePrice':ReverseData[count_B].closePrice, 'lowPrice':ReverseData[count_B].lowPrice, 'highPrice':ReverseData[count_B].highPrice,   'rsi':ReverseData[count_B].rsi, 'flipped':'true'   });
									firebase.database().ref('/Ins/'+queryBook[i][1]+'_B').set({
										ws_key_B: JSON.parse( JSON.stringify(ws_json_B) ),
									})
							...
							if (queryBook[i][1] == 'oJhPznC7LyXGnbnFYLxiTUBbudF3'){
								...
								if ((addThistoSim==1)  && (ws_ws_json_AT.length<2) && (selectCoinForTestAccount=='')  ) { //&& (snapSimBusy!=1)
									selectedCoin =ReverseData[count_B].symbol;
									selectCoinForTestAccount = ReverseData[count_B].symbol;
								
									console.log('ACC TEST UPDATE::: '+ReverseData[count_B].symbol+' Doesnt exist in array so adding.....');
									var _ws_json_=[];
									...
									//_ws_json_.push({'symB':ReverseData[count_B].symbol,  'monitorButtonText':'Stop Monitoring', 'initialPrice':ReverseData[count_B].closePrice, 'discountPercent':'2', 'premiumPercent':'2.9', 'baseBalance':queryBook[i][2], 'chart':ReverseData[count_B].chart, 'chart_c':ReverseData[count_B].chart_c,  'model':ReverseData[count_B].model,   'date':now.format('YYYY-MM-DD HH:mm:ss'), 'origPrice':ReverseData[count_B].price, 'closePrice':ReverseData[count_B].closePrice, 'lowPrice':ReverseData[count_B].lowPrice, 'highPrice':ReverseData[count_B].highPrice,  'rsi':ReverseData[count_B].rsi,  });							
									
									if (l_flipA==1){
										_ws_json_.push({'symB':ReverseData[count_B].symbol,  'monitorButtonText':'Stop Monitoring', 'initialPrice':ReverseData[count_B].closePrice, 'discountPercent':'2', 'premiumPercent':'2.9', 'baseBalance':queryBook[i][2], 'chart':ReverseData[count_B].chart, 'chart_c':ReverseData[count_B].chart_c,  'macd':'N/A', 'model':ReverseData[count_B].model,   'date':now.format('YYYY-MM-DD HH:mm:ss'), 'origPrice':ReverseData[count_B].price, 'rsi':ReverseData[count_B].rsi, '_numTrades':ReverseData[count_B]._numTrades, '_numProfits':ReverseData[count_B]._numProfits, '_expectedProfitPerc':ReverseData[count_B]._expectedProfitPerc, '_biggestLoss':ReverseData[count_B]._biggestLoss, '_netPerRet':ReverseData[count_B]._netPerRet, 'flipped':'no'  });
										...
										_ws_json_.push({'symB':ReverseData[count_B].symbol,  'monitorButtonText':'Stop Monitoring', 'initialPrice':ReverseData[count_B].closePrice, 'discountPercent':'2', 'premiumPercent':'2.9', 'baseBalance':queryBook[i][2], 'chart':ReverseData[count_B].chart, 'chart_c':ReverseData[count_B].chart_c,  'macd':'N/A', 'model':ReverseData[count_B].model,   'date':now.format('YYYY-MM-DD HH:mm:ss'), 'origPrice':ReverseData[count_B].price, 'rsi':ReverseData[count_B].rsi, '_numTrades':ReverseData[count_B]._numTrades, '_numProfits':ReverseData[count_B]._numProfits, '_expectedProfitPerc':ReverseData[count_B]._expectedProfitPerc, '_biggestLoss':ReverseData[count_B]._biggestLoss, '_netPerRet':ReverseData[count_B]._netPerRet, 'flipped':'true'  });
										ws_ws_json_AT.push({'symB':ReverseData[count_B].symbol,  'monitorButtonText':'Stop Monitoring', 'initialPrice':ReverseData[count_B].closePrice, 'discountPercent':'2', 'premiumPercent':'2.9', 'baseBalance':queryBook[i][2], 'chart':ReverseData[count_B].chart, 'chart_c':ReverseData[count_B].chart_c,  'macd':'N/A', 'model':ReverseData[count_B].model,   'date':now.format('YYYY-MM-DD HH:mm:ss'), 'origPrice':ReverseData[count_B].price, 'rsi':ReverseData[count_B].rsi, '_numTrades':ReverseData[count_B]._numTrades, '_numProfits':ReverseData[count_B]._numProfits, '_expectedProfitPerc':ReverseData[count_B]._expectedProfitPerc, '_biggestLoss':ReverseData[count_B]._biggestLoss, '_netPerRet':ReverseData[count_B]._netPerRet, 'flipped':'true'   });
										firebase.database().ref('/Ins/_testACC__S').set({
											ws_key: JSON.parse( JSON.stringify(_ws_json_) ),
										})
							...
							if (queryBook[i][1] == 'oJhPznC7LyXGnbnFYLxiTUBbudF3'){
								...
								if ((addThistoSim==1) && (countLongs<2)   ) { //&& (snapSimBusy!=1)
															
									console.log('SIM UPDATE::: '+ReverseData[count_B].symbol+' Doesnt exist in array so adding.....');
									...
									//short_B = (snapshot.val() && snapshot.val().ws_key_short_B) || [];
									...
									//ws_ws_json_Sim.push({'symB':ReverseData[count_B].symbol,  'monitorButtonText':'Stop Monitoring', 'initialPrice':(ReverseData[count_B].lP_).toString(), 'discountPercent':'2', 'premiumPercent':'2.9', 'baseBalance':queryBook[i][2], 'chart':ReverseData[count_B].chart, 'chart_c':ReverseData[count_B].chart_c,  'macd':'N/A', 'model':ReverseData[count_B].model,   'date':now.format('YYYY-MM-DD HH:mm:ss'), 'origPrice':ReverseData[count_B].price,  'closePrice':ReverseData[count_B].closePrice, 'lowPrice':ReverseData[count_B].lowPrice, 'highPrice':ReverseData[count_B].highPrice, 'rsi':ReverseData[count_B].rsi,  });							
			...
			//if ((selectedCoin=='')  && (queryBook[i][8]=='1')  &&  (queryBook[i][9]<2)  ){ //[i][8] is actually toggleSwitchShort:  //&& ((s_snapSimBusy!='1') || ((s_snapSimBusy=='1') && (s_snapActSim=='0') ) )  ///////((s_snapSimBusy!='1') || ((s_snapSimBusy=='1') && (s_snapActSim=='0') ) )
			//	console.log('__________________________________________________')
			//	console.log('IT WAS STILL EMPTY SO IT WILL SHORT...');
			//	console.log('selectedCoin:'+selectedCoin+' & i8:'+queryBook[i][8]+' & i9'+queryBook[i][9])
			//	queryBook.push(['SHORT_COIN', queryBook[i][1], queryBook[i][2], queryBook[i][3], queryBook[i][4], queryBook[i][5],   ]); //7 & 8..///queryBook[i][8]
			//}
			//else {
			//	console.log('A NOTHING HAPPENED AND NOT GOING TO STILL SHORT...')
			//	console.log('selectedCoin:'+selectedCoin+' & i8:'+queryBook[i][8]+' & i9'+queryBook[i][9])
			//}
			
	  }
	  else{
		  //if ((s_snapSimBusy!='1') || ((s_snapSimBusy=='1') && (s_snapActSim=='0') ) ) {
			  // (selectedCoin=='')  &&
			
			//if ( (queryBook[i][8]=='1') &&  (queryBook[i][9]<2)  ){ //[i][8] is actually toggleSwitchShort:  //&& ((s_snapSimBusy!='1') || ((s_snapSimBusy=='1') && (s_snapActSim=='0') ) )
			...
			//}
			//else {
			//	console.log('B NOTHING HAPPENED AND NOT GOING TO STILL SHORT...')
			//	console.log('selectedCoin:'+selectedCoin+' & i8:'+queryBook[i][8]+' & i9'+queryBook[i][9])
			//}
	  }
